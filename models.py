...
"""
数据库模型：
利用ORM模型将数据库中各个表抽象为实体类，其余模块通过利用属性进行增删改查操作
"""
...
class Bus_passenger(db.Model):
    ...
    bus_passenger_id = db.Column(db.Integer, primary_key=True, autoincrement=True)
...
class Passenger(db.Model):
    ...
    face_landmark = db.Column(db.CHAR(10))
    ...
    def __init__(self, passenger_id, delete_type=0, sex=0,
                 age_level=1, is_glasses=0, is_hat=0, is_mask=0,
                 is_lipstick=0, face_expression=0, future_matrix=None):
        self.passenger_id = passenger_id
        self.delete_type = delete_type
        self.sex = sex
...
class Station(db.Model):
    ...
    # density = db.Column(db.String(11), nullable=False, comment='(G:少，B:较多，R:多)')
    ...
    def __init__(self, station_name, station_type, longitude, latitude, delete_type=0):
...
class Station_passenger(db.Model):
    ...
    station_passenger_id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    station_id = db.Column(db.Integer, db.ForeignKey("station.station_id"))
    passenger_id = db.Column(db.Integer, db.ForeignKey("passenger.passenger_id"))
    timestamp = db.Column(db.TIMESTAMP)
...
class User_bus(db.Model):
    ...
    user_bus_id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    user_id = db.Column(db.Integer, db.ForeignKey("user.user_id"), comment='用户编号')
    bus_id = db.Column(db.Integer, db.ForeignKey("bus.bus_id"))
    timestamp = db.Column(db.TIMESTAMP)
...
class User_passenger(db.Model):
    ...
    user_passenger_id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    user_id = db.Column(db.Integer, db.ForeignKey("user.user_id"), comment='用户编号')
    passenger_id = db.Column(db.Integer, db.ForeignKey("passenger.passenger_id"))
    timestamp = db.Column(db.TIMESTAMP)
...
class User_station(db.Model):
    ...
    user_station_id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    user_id = db.Column(db.Integer, db.ForeignKey("user.user_id"), comment='用户编号')
    station_id = db.Column(db.Integer, db.ForeignKey("station.station_id"))
    timestamp = db.Column(db.TIMESTAMP)
