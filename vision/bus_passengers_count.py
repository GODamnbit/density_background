...
'''
实现功能:展示某路公交车上人群数量随时间的变化情况折线图
调用函数：show_bus_passengers_count_ply(bus_id,start_date,end_date)
'''

#设置字体
myfont = fm.FontProperties(fname="C:\Windows\Fonts\AdobeHeitiStd-Regular.otf")

def bus_passengers_count_(bus_id,start_date,end_date):
    # 获取数据
    ...
    df = df.join(count_df)
    # 去除没有bus_id的乘客
    new_df = df[pd.notnull(df["bus_id"])]
    # 去除没有时间戳的乘客
    new_df = new_df[pd.notnull(df["timestamp"])]
    # 选择某号站台的乘客
    new_df = new_df[new_df["bus_id"] == bus_id]
    # 选择起始时间之后的数据
...
def show_bus_passengers_count_mpl(bus_id,start_date,end_date):
    df = bus_passengers_count_(bus_id,start_date,end_date)
    ...
    # # 图的标题
    # plt.title("723路公交车上一天内不同时刻的人群数量统计", fontproperties=myfont)
    # plt.savefig("723路公交车上一天内不同时刻的人群数量统计.jpg")
    # plt.show()
    ...
    iris_im = """<h1>723路公交车上一天内不同时刻的人群数量统计直方图</h1>  """ + """<img src="%s">""" % imd
    ...
    tree.write('某路公交车上一天内不同时刻的人群数量统计直方图.html')

    # 最后使用默认浏览器打开 html 文件
    import webbrowser
    webbrowser.open('某路公交车上一天内不同时刻的人群数量统计直方图.html', new=1)


def show_bus_passengers_count_ply(bus_id,start_date,end_date):
    '''
    ...
    fig = go.Figure()
    # 绘制折线图
    fig.add_trace(go.Scatter(x=_x, y=_y, name='Bus '+str(bus_id),
                             line=dict(color='firebrick', width=4)))
    # 设置图表布局
    fig.update_layout(title=str(bus_id) + '路公交车上人群数量随时间的变化情况折线图',
                      title_font_size=40,
    ...
    # 使用离线的接口，生成离线html
    # pltoff.plot(fig, filename=str(bus_id)+"路公交车上人群数量随时间的变化情况.html")
    # 将生成的HTML通过字符串的形式返回
...
if __name__ == '__main__':
    # show_bus_passengers_count_mpl(723,"2020-04-05 00:00:00","2020-04-06 00:00:00")
    show_bus_passengers_count_ply(723, "2020-04-05 00:00:00","2020-04-06 00:00:00")