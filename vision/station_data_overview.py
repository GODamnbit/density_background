...
'''
实现功能:展示不同站台的人群数量随时间的变化情况折线图
调用函数：show_station_date_overview_ply(start_date, end_date)
'''

#设置字体
myfont = fm.FontProperties(fname="./vision/AdobeHeitiStd-Regular.otf")

def show_station_date_overview_mpl(start_date,end_date):
    global temp_grouped
    ...
    #不同站台的乘客的数量和时间的对应关系并不相同,需要先统一统计的时间,没有的时间段填充0
    ...
    #分组
    for station,grouped in new_df.groupby(by=["station_id"]):
        ...
        #绘制散点图,但是效果不明显
        # ax.scatter(_x, _y,
        #            c=colors[station_list.index(station)],
        #            alpha=0.5,
        #            label=station
        #            )
        #绘制折线图
    ...
    # 解决xticklable时间带年月日
    xticklables = [i.strftime('%H:%M') for i in temp_grouped.index]
    # 解决xticklable刻度太密集
    plt.xticks(range(0, len(temp_grouped.index), 2), xticklables[::2], rotation=45)
    ...
    # 图的标题
    # plt.title("不同站台的人群数量随时间的变化情况",fontproperties=myfont)
    # plt.savefig("不同站台的人群数量随时间的变化情况.png")
    # plt.show()
    ...
    iris_im = """<h1>不同站台的人群数量随时间的变化情况折线图</h1>  """ + """<img src="%s">""" % imd
    ...
    tree.write('不同站台的人群数量随时间的变化情况折线图.html')

    # 最后使用默认浏览器打开 html 文件
    import webbrowser
    webbrowser.open('不同站台的人群数量随时间的变化情况折线图.html', new=1)


def show_station_date_overview_ply(start_date, end_date):
    '''
    展示不同站台的人群数量随时间的变化情况折线图
    ...
    for station, grouped in new_df.groupby(by=["station_id"]):
        ...
        fig.add_trace(go.Scatter(x=_x, y=_y,
                                 name=station,
                                 mode='lines+markers',
                                 line=dict(color=colors[station_list.index(station)], width=4)))

    # 设置图表布局
    fig.update_layout(title='不同站台的人群数量随时间的变化情况折线图',
                      title_font_size=40,
    ...
    # 使用离线的接口，生成离线html
    # pltoff.plot(fig, filename="不同站台的人群数量随时间的变化情况折线图.html")
    # 将生成的HTML通过字符串的形式返回
...
if __name__ == '__main__':
    ...
    # show_station_date_overview_mpl(start,end)
    show_station_date_overview_ply(start,end)
